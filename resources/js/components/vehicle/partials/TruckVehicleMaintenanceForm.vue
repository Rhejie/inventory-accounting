...
<script>
export default {
    name: 'TruckVehicleMaintenanceForm',
    props: {
        id: null,
        mode: null,
        model : {}
    ...
    created() {
        ...
        if(this.model && this.model.id && this.mode == 'update') {
            ...
            if(this.model.file) {
                this.fileList = [{name: this.model.file_name, url: `/storage/truck_vehicle/maintenance_attachment/${this.model.file}`}]
            }
        }
    },
    methods: {
        ...
        async storeJob() {
            try {
                ...
                if(this.ruleForm.truck_vehicle_id)formData.append('truck_vehicle_id', this.ruleForm.truck_vehicle_id)
                ...
                const res = await this.$API.TruckVehicleMaintenance.storeMaintenance(formData)
                ...
                this.$EventDispatcher.fire('ADD_NEW_TRUCK_MAINTENANCE', res.data);
            } catch (error) {
                console.log(error);
                this.disabledButton = false;
            }
        ...
        async updateJob() {
            try {
                ...
                if(this.ruleForm.assigned)formData.append('assigned', this.ruleForm.assigned)
                if(this.ruleForm.file_name)formData.append('file_name', this.ruleForm.file_name)
                const res = await this.$API.TruckVehicleMaintenance.updateMaintenance(this.model.id, formData);
                this.$notify({
                    title: 'Success',
                    message: 'Successfully Updated!',
                    type: 'success'
                });
                this.$EventDispatcher.fire('UPDATED_TRUCK_MAINTENANCE', res.data);
                this.disabledButton = false;
            } catch (error) {
                console.log(error);
                this.disabledButton = false;
        ...
        async getMaintenanceType() {
            try {
                ...
                const res = await this.$API.TruckVehicleMaintenance.getMaintenanceType();
    ...
    watch: {
        model(newVal, oldVal) {
            if(newVal != oldVal & this.mode == 'update') {
                ...
                if(newVal.file) {
                    this.fileList = [{name: newVal.file_name, url: `/storage/truck_vehicle/maintenance_attachment/${newVal.file}`}]
                }
            }
        },
        mode(val) {
