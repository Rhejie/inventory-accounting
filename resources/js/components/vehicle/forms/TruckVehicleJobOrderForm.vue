    ...
    methods: {
        ...
        async storeJob() {
            try {
                ...
                const res = await this.$API.TruckVehicleJobOrder.storeJobOrder(this.ruleForm)
                this.$notify({
                    title: 'Success',
                ...
                this.$EventDispatcher.fire('ADD_NEW_TRUCK_JOB_ORDER', res.data);
        ...
        async updateJob() {
            try {
                this.disabledButton = true
                this.ruleForm.date_created = this.$df.formatDate(this.ruleForm.date_created, "YYYY-MM-DD")
                const res = await this.$API.TruckVehicleJobOrder.updateJobOrder(this.model.id, this.ruleForm);
                this.$notify({
                    title: 'Success',
                    message: 'Successfully Updated!',
                    type: 'success'
                });
                this.$EventDispatcher.fire('UPDATED_TRUCK_JOB_ORDER', res.data);
                this.disabledButton = false;
            } catch (error) {
                console.log(error);
                this.disabledButton = false;
        ...
        async getJobOrderType() {
            try {
                ...
                const res = await this.$API.TruckVehicleJobOrder.getJobOrderType();
