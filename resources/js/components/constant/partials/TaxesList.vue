...
export default {
    ...
    created() {
        ...
        this.$EventDispatcher.listen('NEW_TAX', data => {
            this.taxesData.unshift(data)
            this.dialogFormVisible = false
            this.mode = ''
            this.model = {}
        })

        this.$EventDispatcher.listen('UPDATE_TAX', data => {
    ...
    methods: {
        async getTaxes() {
            try {
                ...
                const res = await this.$API.Constant.getTaxes(params);
                this.taxesData = res.data.data
                this.taxesPagination = res.data
                this.loading = false;
            } catch (error) {
                console.log(error);
            }
        ...
        async delete(index, data) {
            try {
                await this.$API.Constant.deleteTax(data.id)
                this.$notify({
                    title: 'Success',
