        ...
        <el-form :model="ruleForm" label-position="top" :rules="rules" ref="ruleForm" class="demo-ruleForm">
            ...
            <el-form-item>
                <div style="float:right">
                    ...
                    <el-button  v-if="mode != 'update'" :disabled="disabledButton" @click="resetForm('ruleForm')">Reset</el-button>
                </div>
            </el-form-item>
        </el-form>
    </div>
...
<script>
export default {
    name: 'HeavyEquipmentMaintenanceForm',
    props: {
        id: null,
        mode: null,
        model : {}
    ...
    created() {
        ...
        if(this.model && this.model.id && this.mode == 'update') {
            ...
            if(this.model.file) {
                this.fileList = [{name: this.model.file_name, url: `/storage/heavy_equipments/maintenance_attachment/${this.model.file}`}]
            }
        }
    },
    methods: {
        ...
        async storeJob() {
            try {
                ...
                if(this.ruleForm.heavy_equipment_id)formData.append('heavy_equipment_id', this.ruleForm.heavy_equipment_id)
                ...
                const res = await this.$API.HeavyEquipmentMaintenance.storeMaintenance(formData)
                ...
                this.$EventDispatcher.fire('ADD_NEW_HEAVY_MAINTENANCE', res.data);
            } catch (error) {
                console.log(error);
                this.disabledButton = false
            }
        ...
        async updateJob() {
            try {
                ...
                if(this.ruleForm.assigned)formData.append('assigned', this.ruleForm.assigned)
                if(this.ruleForm.file_name)formData.append('file_name', this.ruleForm.file_name)
                const res = await this.$API.HeavyEquipmentMaintenance.updateMaintenance(this.model.id, formData);
                this.$notify({
                    title: 'Success',
                ...
                this.disabledButton = false
                this.$EventDispatcher.fire('UPDATED_HEAVY_MAINTENANCE', res.data);
            } catch (error) {
                console.log(error);
                this.disabledButton = false
            }
        ...
        async getMaintenanceType() {
            try {
                ...
                const res = await this.$API.HeavyEquipmentMaintenance.getMaintenanceType();
    ...
    watch: {
        model(newVal, oldVal) {
            if(newVal != oldVal & this.mode == 'update') {
                ...
                if(newVal.file) {
                    this.fileList = [{name: newVal.file_name, url: `/storage/heavy_equipments/maintenance_attachment/${newVal.file}`}]
                }
            }
        },
        mode(val) {
