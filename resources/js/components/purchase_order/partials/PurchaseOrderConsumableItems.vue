            ...
            <tbody class="tbody_po" v-loading="loading">
                <tr class="po_item_data_item" v-for="(item, index) in poItemData" :key="index">
                    ...
                    <td class="item_center_items">
                        ...
                        <div v-else>
                            <el-input type="number" v-model="item.unit_price" @change="unitPrice(item, $event, index)" placeholder="Unit Price..."></el-input>
                        </div>
                    </td>
                    <td class="item_center_items">
                        <div v-if="!isEdit">
                            {{item.amount | addComma}}
                        </div>
                        <div v-else>
                            <el-input disabled v-model="item.amount" placeholder="Amount..."></el-input>
                        </div>
                    </td>
                    <td class="button_center_item buttonPrint">
                        ...
                        <div v-if="isEdit">
                            <el-button size="mini" type="danger" @click="handleCancel"> Remove</el-button>
                        </div>
                    </td>
                </tr>

                <!-- FORM -->
                <tr class="fomr_table_po_item buttonPrint" v-if="poItemData.length > 0">
                    <td colspan="3">
                        <el-select
                ...
                <tr class="po_footer_items">
                    ...
                    <td>
                        {{purchaseOrder.subtotal | addComma}}
                    </td>
                </tr>
                <tr class="po_footer_items">
                    <td>
                ...
                <tr class="po_footer_items">
                    ...
                    <td>
                        <el-popover
                            ...
                            <el-button type="text" slot="reference">{{purchaseOrder.discount | addComma}}</el-button>
                        </el-popover>
                    </td>
                </tr>
                <tr class="po_footer_items">
                    ...
                    <td>
                        {{purchaseOrder.subtotal_net | addComma}}
                    </td>
                </tr>
                <tr class="po_footer_items">
                    <td colspan="3"></td>
                    ...
                    <td>
                        <el-popover
                            ...
                            <el-button type="text" slot="reference">{{purchaseOrder.others | addComma}}</el-button>
                        </el-popover>
                    </td>
                </tr>
                <tr class="po_footer_items">
                    ...
                    <td colspan="3">
                        ...
                        <div style="float:right">
                            <span>{{purchaseOrder.total_net | addComma}}</span>
                        </div>
                    </td>
                </tr>
            </tbody>
...
export default {
    name: 'PurchaseOrderConsumableItem',
    mixins: [paginate],
    props: {
        id: null,
        supplierId: null,
    ...
    created() {
        ...
        this.$EventDispatcher.listen('PO_CONSUMABLE_ITEMS', data => {
            this.poItemData = data.map(item => {
                return {
                    consumable_item: {
                        item : item.consumable_supply.item_name
                    },
                    quantity: item.quantity,
                    unit : item.consumable_supply.unit_of_measure,
                    unit_price: '',
                }
            })
            this.isEdit = true
    ...
    methods: {
        unitPrice(item, amount, index) {
            let amountTotal = parseInt(item.quantity) * parseInt(amount)
            this.poItemData[index].amount = amountTotal
            return
        },
    ...
    watch: {
        ...
        poItemData(newVal, oldVal) {
            if(newVal != oldVal) {
                this.purchaseOrder.consumableItems = newVal;
            }
        },
        'form.item_id'(value) {
