            ...
            <div class="container-fluid">
                <!--begin::Row-->
                    <el-card class="box-card purchase_order_wrapper" shadow="never" id="printMe">
                        <div class="text item">
                            ...
                            <el-descriptions :column="2" >
                                ...
                                <el-descriptions-item label="MRS REF NO" label-class-name="my-label">
                                    ...
                                    <template v-else>
                                        <el-select
                                            v-model="purchaseOrder.projematerial_reference_noct_id"
                                            style="width: 100%"
                                            filterable
                                            remote
                                            reserve-keyword
                                            ...
                                            <el-option
                                            v-for="item in materialsRefNumbers"
                                            :key="item.id"
                        ...
                        <div class="po_item">
                            <purchase-order-consumable-items :id="id" :errors_purchase_order="errors" :doneEdit="doneEdit" :supplier_id="purchaseOrder.supplier_id" :purchase-order="purchaseOrder" :consumableItems="consumableItems"></purchase-order-consumable-items>
                        </div>
                    </el-card>
...
export default {
    ...
    methods: {
        async remoteMaterialMethod(query) {
            if (query !== '') {
                ...
                try {
                    ...
                    this.materialsRefNumbers = res.data
        ...
        async changeMaterial(id) {
            let items = this.materialsRefNumbers.find(rm => rm.id == id)
            this.consumableItems = [] = items.request_material_consumable_item
            let params = {
                supplier_id : this.purchaseOrder.supplier_id,
                consumable_item : this.consumableItems
            }
            const res = await this.$API.Consumable.checkItem(params);
            console.log(res.data);
            this.$EventDispatcher.fire('PO_CONSUMABLE_ITEMS', this.consumableItems)
            this.purchaseOrder.material_reference_no = items.request_material_code
        },
