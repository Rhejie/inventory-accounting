...
export default {
    ...
    created() {
        ...
        this.$EventDispatcher.listen('handleCloseMaintenanceType', data => {
            this.resetForm('form')
        })
    },
    methods: {
        async storeType() {
            ...
            try {
                const res = await this.$API.MaintenanceType.store(this.form);

                this.$EventDispatcher.fire('NEW_MAINTENANCE_TYPE', res.data);
                this.$notify({
                    title: 'Success',
        ...
        async updateType() {
            ...
            try {
                ...
                const res = await this.$API.MaintenanceType.update(this.model.id, this.form);

                this.$EventDispatcher.fire('UPDATE_MAINTENANCE_TYPE', res.data);
                this.$notify({
                    title: 'Success',
